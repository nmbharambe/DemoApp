...
export function dateFormatter(params) {
  if (
    ...
    if (str === 'Invalid date') str = params.value;
    return str;
  } else return '';
}
...
export function dateTimeFormatter(params) {
  if (params.value == undefined || params.value == '0001-01-01T00:00:00')
    return '';
  else return moment(params.value).format('DD/MM/YYYY HH:mm');
}

export function amountFormatter(params) {
  if (
    params.value != undefined &&
    Number(Number(params.value).toFixed(2)) != 0
  ) {
    if (Number.isInteger(Number(Number(params.value).toFixed(2)))) {
      // Don't show trailing 0's if number rounded off to 2 decimals is an integer
      ...
      return Number(params.value).toLocaleString(undefined, {
        // Show 2 trailing digits if non integer
...
export function noDecimalAmountFormatter(params) {
  if (
    params.value != undefined &&
    Number(Number(params.value).toFixed(0)) != 0
  ) {
    return Number(params.value).toLocaleString(undefined, {
      minimumFractionDigits: 0,
      maximumFractionDigits: 0,
...
export function removeDecimalFormatter(params) {
  ...
  } else if (Number(params.value) == 0) return '-';
  else return '';
}
